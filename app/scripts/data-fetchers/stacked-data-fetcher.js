...
function assertTilesetsAreStackable(infos) {
  ...
  for (const info of rest) {
    ...
    assert(
      first.coordSystem === info.coordSystem,
      'tileset coordSystem mismatch',
    );
    first.resolutions.forEach((resolution, index) => {
      assert(
        resolution === info.resolutions[index],
        'tileset resolution mismatch',
      );
    });
  ...
  return {
    ...
    async fetchTilesDebounced(receivedTiles, tileIds) {
      ...
      if (neededTileIds.length > 0) {
        const tiles = await dataFetcher.fetchTilesDebounced(() => {},
        neededTileIds);
...
export default class StackedDataFetcher {
   ...
   */
  async fetchTilesDebounced(receivedTiles, tileIds) {
    return this.current.fetchTilesDebounced(receivedTiles, tileIds);
  }
}
